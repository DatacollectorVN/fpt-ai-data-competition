...
def resize_with_padding(img, anno, expected_size):
    ...
    for line in lines:
        ...
        obj_new = "{} {} {} {} {}".format(
            int(obj[0]),
            (float(obj[1]) * img.size[0] + pad_width) / expected_size[0],
            (float(obj[2]) * img.size[1] + pad_height) / expected_size[1],
            float(obj[3]) * img.size[0] / expected_size[0],
            float(obj[4]) * img.size[1] / expected_size[1],
        )
...
if __name__ == "__main__":
    IMAGE_DIR = "./data/dataset/images/val_new"
    LABEL_DIR = "./data/dataset/labels/val_new"
    ...
    for image_file in glob.glob(os.path.join(IMAGE_DIR, "*.jpg")):
        ...
        img_new, annos_list = resize_with_padding(
            img, label_file, (1280, 1280))
        print("Succeesed with file:", image_file.split("/")[-1])
        img_new.save(IMAGE_DIR + "_padding/" + image_file.split("/")[-1].split(".")[0] + "_PADDING.jpg", quality=90, optimize=True, progressive=True,
                     )
